...
import { aiPreferenceApi, type AIPreferenceResponse } from "./services/aiPreferenceApi";
...
type ViewKey = "overview" | "behavior" | "funnel" | "products" | "profiles" | "aiPreferences" | "alerts" | "recommendations" | "reports" | "audit";
...
const navItems: Array<{ key: ViewKey; label: string; caption: string }> = [
  ...
  { key: "aiPreferences", label: "AI偏好识别", caption: "模型权重分类" },
...
const portalProfiles: PortalProfile[] = [
  {
    ...
    scopeLabel: "全平台数据",
    workspace: "internal",
    allowedViews: ["overview", "behavior", "funnel", "products", "profiles", "aiPreferences", "alerts", "recommendations", "reports", "audit"],
  ...
  },
  {
    ...
    scopeLabel: "运营全域数据",
    workspace: "internal",
    allowedViews: ["overview", "behavior", "funnel", "products", "profiles", "aiPreferences", "alerts", "recommendations", "reports"],
    defaultView: "overview",
    canOperate: true,
    canExport: true,
  },
  {
    ...
    scopeLabel: "指定分群数据",
    workspace: "internal",
    allowedViews: ["overview", "behavior", "funnel", "products", "profiles", "aiPreferences", "recommendations", "reports"],
    defaultView: "funnel",
    canOperate: false,
    canExport: true,
  },
...
function App() {
  ...
  const [aiPreferences, setAiPreferences] = useState<AIPreferenceResponse | null>(null);
  ...
  const refreshData = useCallback(async () => {
    ...
    if (portal.allowedViews.includes("aiPreferences")) tasks.push(["AI偏好", aiPreferenceApi.preferenceCategories()]);
    ...
    settled.forEach((result, index) => {
      ...
      if (label === "AI偏好") setAiPreferences(result.value as AIPreferenceResponse);
      ...
      <main className="workspace">
        {portal.workspace === "internal" ? (
          <>
            ...
            {activeView === "aiPreferences" && <AIPreferenceView result={aiPreferences} />}
...
function AIPreferenceView({ result }: { result: AIPreferenceResponse | null }) {
  const items = result?.items ?? [];
  const topCategories = items.flatMap((item) => item.categories.slice(0, 1));
  const avgConfidence = topCategories.length
    ? Math.round(topCategories.reduce((sum, item) => sum + item.confidence, 0) / topCategories.length)
    : 0;
  const modelLoaded = result?.model.loaded;
  const modelDisplayName = formatAIModelName(result?.model.name);
  const inferenceMode = result?.model.mode === "sentence-transformers" ? "语义向量" : "兜底向量";
    ...
    <div className="ai-layout">
      <section className="metric-grid full-span">
        <article className="metric-card">
          <span>模型权重</span>
          <strong>{modelLoaded ? "已加载" : "待加载"}</strong>
          <em className={modelLoaded ? "good" : "warn"}>{modelDisplayName}</em>
        </article>
        <article className="metric-card">
          <span>推理模式</span>
          <strong className="compact-value">{inferenceMode}</strong>
          <em>{result?.model.mode ?? "-"}</em>
        </article>
        <article className="metric-card">
          <span>识别用户</span>
        ...
        <article className="metric-card">
          ...
          <em className={avgConfidence >= 70 ? "good" : "warn"}>Top-1 品类</em>
        </article>
      </section>

      <section className="panel full-span ai-model-panel">
        <PanelTitle title="AI 品类偏好模型" meta="预训练语义向量 + 行为权重聚合 + 品类相似度分类" />
        <div className="ai-model-strip">
          ...
          <div>
            <span>权重状态</span>
            <strong>{modelLoaded ? "预训练权重已加载" : "等待加载权重"}</strong>
          </div>
          <div>
            <span>生成时间</span>
            <strong>{formatTime(result?.generatedAt ?? "")}</strong>
          </div>
        </div>
        {result?.model.loadError && <div className="notice error">模型权重加载失败，当前使用兜底向量：{result.model.loadError}</div>}
        <div className="weight-list">
          {Object.entries(result?.behaviorWeights ?? {}).map(([action, weight]) => (
            <span key={action}>{eventLabels[action] ?? action}：{weight}</span>
          ))}
        </div>
      </section>

      {items.length ? (
        <section className="ai-user-grid full-span">
          {items.map((item) => (
            <article className="ai-user-card" key={item.userId}>
              <div className="ai-user-head">
                <div>
                  <span>用户</span>
                  <strong>{item.userId}</strong>
                </div>
                <div className="score-ring small">{Math.round(item.primaryConfidence)}</div>
              </div>
              <p>AI 判断该用户最可能购买 <b>{item.primaryCategory}</b> 类商品。</p>
              <div className="ai-category-list">
                {item.categories.map((category) => (
                  <div className="ai-category-row" key={category.category}>
                    <div>
                      <strong>{category.category}</strong>
                      <span>语义相似度 {category.semanticSimilarity} · 行为分 {category.behaviorScore}</span>
                    </div>
                    <b>{Math.round(category.confidence)}%</b>
                    <div className="confidence"><i style={{ width: `${Math.min(category.confidence, 100)}%` }} /></div>
                    <div className="evidence-list">
                      {category.evidenceProducts.map((product) => (
        ...
        <section className="panel full-span">
          <EmptyState text="暂无可识别的用户行为。用 user001、user002、user003 登录并浏览、收藏、加购或购买商品后，AI 模型会生成品类偏好。" />
        </section>
      )}
    </div>
  );
...
function formatAIModelName(value?: string) {
  if (!value) return "BAAI/bge-small-zh-v1.5";
  if (value.includes("bge-small-zh-v1")) return "BAAI/bge-small-zh-v1.5";
  return value.includes("\\") || value.includes("/") ? "BAAI/bge-small-zh-v1.5" : value;
}
