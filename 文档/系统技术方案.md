...
| 层级 | 技术 | 说明 |
|---|---|---|
| 前端 | React + TypeScript + Vite | 支持复杂仪表盘、用户购物端和多角色交互 |
...
本系统以 FastAPI + React + MySQL 为核心架构，围绕用户行为采集、实时指标分析、转化漏斗、用户画像、智能推荐、权限审计和报表统计构建。

技术路线适配性：

1. FastAPI 提供清晰的 API 分层、参数校验和 Swagger/OpenAPI 文档。
2. Python 生态适合实现用户画像、推荐算法、异常检测等分析逻辑。
3. React + TypeScript + Vite 适合构建多角色工作台、购物端和复杂图表交互。
4. MySQL 8.x 作为主数据库，承载用户、商品、行为、订单、支付、画像和审计数据。
5. SQLAlchemy + Alembic 负责实体映射、查询组织和数据库结构版本管理。
